...
function main {
    ...
    do
        export WIDTH_MULT=$(echo ${model_name} |cut -d '-' -f 2)
        export CKPT_DIR=$(echo ${model_name} |sed 's/[\.-]/_/g;s/$/.pth/')
        # cache
        python imagenet.py -e --device $device \
            -a hbonet --width-mult $WIDTH_MULT --input-size 224 \
            -d $DATASET_DIR --weight pretrained/$CKPT_DIR \
            --batch-size 1 \
